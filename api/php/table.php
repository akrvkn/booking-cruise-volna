...
//echo $ships_list[$entry].PHP_EOL;
            	$tours = json_decode(file_get_contents($base_dir.'/'.$entry.'/tours.json'), true);
            	if($tours){
            		foreach($tours as $tour_id=>$tour){
            			$all++;
						if(mb_stristr($tour['cities'], 'Москва', false, 'UTF-8') || mb_stristr($tour['cities'], 'Петербург', false, 'UTF-8')){
							//echo $entry.'->'.$ships[$entry].PHP_EOL;
							if(!in_array($ships_list[$entry], $ships_arr)){
								$ships_arr[] = $ships_list[$entry];
							}
...
$executionEndTime5 = microtime(true);

$seconds5 = $executionEndTime5 - $executionStartTime;
 
file_put_contents('logs/table.log', "Выполнено ".date('Y-m-d H:i:s')." за $seconds5 секунд.");
?>
